...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True, # Changed: now required
...
The output of one agent becomes the input for the next agent in the chain."""
    ...
    parser.add_argument(
        ...
        help="If present, the output of the last agent is used as input for the first agent, and the loop continues indefinitely."
    )
    parser.add_argument(
        "--no-history",
        action="store_true", # This flag will be True if present, False otherwise
    ...
    if not args.agent:
        print(f"{Fore.RED}Error: At least one agent must be defined using the --agent argument.{Style.RESET_ALL}", file=sys.stderr)
        parser.print_help()
        sys.exit(1)
    ...
    if len(args.model) == 1:
        ...
        print(f"--- Multiple Ollama models specified: {args.model}. Each agent will use a corresponding model. ---")
        if len(args.model) < len(args.agent):
            print(f"{Fore.YELLOW}Warning: You have defined {len(args.agent)} agents but only {len(args.model)} models.", file=sys.stderr)
            print(f"The remaining agents will use the last specified model: '{args.model[-1]}'.{Style.RESET_ALL}", file=sys.stderr)
    ...
    # Initialize or reset chat history for the current chain run (not accumulated across loops)
    # This history tracks the interactions *within* one full pass of the agent chain.
    chain_run_history = []

    # Main loop for continuous execution if --loop flag is present
    while True:
        loop_iteration += 1
        print(f"\n{Fore.WHITE}{Style.BRIGHT}--- {'Starting Loop Iteration' if args.loop else 'Starting Chain'} {loop_iteration if args.loop else ''} ---{Style.RESET_ALL}")

        # Input for the first agent. If looping, it's the output of the previous loop's last agent.
        # Otherwise, it's the initial task.
        current_input_for_agent = initial_task if loop_iteration == 1 else final_output_from_prev_iteration
        ...
        for i, agent_str in enumerate(args.agent):
            ...
            try:
                ...
                if not role or not pre_prompt:
                    raise ValueError("Role and pre-prompt cannot be empty after stripping whitespace.")
            except ValueError as e:
                print(f"{Fore.RED}Error: Invalid agent format '{agent_str}'. Expected 'Role:Pre-prompt'. Details: {e}{Style.RESET_ALL}", file=sys.stderr)
                sys.exit(1)
            ...
            if len(args.model) == 1:
                ...
                print(f"{Fore.YELLOW}  Note: Agent {i+1} is using the last specified model '{current_model_name}' as not enough unique models were provided.{Style.RESET_ALL}", file=sys.stderr)
            ...
            try:
                ...
                print(f"{current_color}  Attempting to connect to Ollama model '{current_model_name}'...{Style.RESET_ALL}")
                ...
                print(f"{Fore.RED}Error connecting to Ollama or model '{current_model_name}' not found for Agent {i+1}: {e}{Style.RESET_ALL}", file=sys.stderr)
                print(f"{Fore.RED}Please ensure Ollama is running and the specified model is pulled (e.g., 'ollama pull llama2').{Style.RESET_ALL}", file=sys.stderr)
                sys.exit(1)
            ...
            if not args.no_history and chain_run_history:
                composed_prompt_parts.append("\n--- Conversation History in this Chain Run ---\n")
                for hist_entry in chain_run_history:
                    # Simple formatting for OllamaLLM to understand a "chat" format
                    composed_prompt_parts.append(f"{hist_entry['role'].capitalize()}: {hist_entry['content']}\n")
                composed_prompt_parts.append("------------------------------------------\n")

            composed_prompt_parts.append(f"{pre_prompt}\n\nYour task as {role} is to process the following input:\n{current_input_for_agent}")
            full_prompt_text = "".join(composed_prompt_parts)

            print(f"{current_color} Prompt for '{current_model_name}' for Agent {i+1} ({role})...{Style.RESET_ALL}:\n\n{full_prompt_text}{Style.RESET_ALL}")
            prompt_template = PromptTemplate.from_template(full_prompt_text)

            print(f"{current_color}  Generating response using model '{current_model_name}' for Agent {i+1} ({role})...{Style.RESET_ALL}")
            
            # --- START: Streaming Implementation ---
            response_chunks = []
            response_text = ""
            print(f"\n{current_color}{Style.BRIGHT}--- Agent {i+1} ({role}) Response (Streaming) ---{Style.RESET_ALL}\n{current_color}{Style.BRIGHT}", end="")
            try:
                # Use llm.stream() instead of llm.invoke() for real-time output
                print(f"{current_color}", end="")
                for chunk in llm.stream(prompt_template.format()):
                    # The chunk directly contains the string content for OllamaLLM
                    print(f"{current_color}{Style.BRIGHT}{chunk}{Style.RESET_ALL}", end="", flush=True) # Print chunk and flush buffer
                    response_chunks.append(chunk) # Append the string chunk directly
                response_text = "".join(response_chunks) # Join all chunks to get the full response
                print(f"{Style.RESET_ALL}\n") # Reset style and add a final newline after streaming
            except Exception as e:
                print(f"{Fore.RED}Error during LLM streaming invocation for Agent {i+1} ({role}): {e}{Style.RESET_ALL}", file=sys.stderr)
                print(f"{Fore.RED}Please check the Ollama server logs for more details.{Style.RESET_ALL}", file=sys.stderr)
                sys.exit(1)
            # --- END: Streaming Implementation ---

            # Update history for the current chain run if --no-history is NOT present
            if not args.no_history:
                print(f"{Fore.YELLOW}Add hitory.{Style.RESET_ALL}", file=sys.stderr)
                # Add the agent's 'query' (pre-prompt + input) as a 'user' turn
                chain_run_history.append({"role": role, "content": f"Role: {role}, History: {current_input_for_agent}"})
                # Add the agent's response as an 'assistant' turn
                chain_run_history.append({"role": role, "content": f"Response: {response_text}"})
        ...
        print(f"\n{Fore.GREEN}--- Chain execution complete for Loop Iteration {loop_iteration if args.loop else ''} ---{Style.RESET_ALL}")
        print(f"{Fore.GREEN}Final output of this iteration:\n{final_output_from_prev_iteration}{Style.RESET_ALL}")

        if not args.loop:
            break # Exit the loop if --loop flag is not present
        else:
            print(f"\n{Fore.WHITE}Sleeping for a few seconds before next loop iteration... (Press Ctrl+C to stop){Style.RESET_ALL}")
            time.sleep(5) # Small delay to make loops readable and prevent rapid API calls
